...
def sine(x, fs, f=1):
    # print(x)
    return (np.sin(2 * np.pi * f * (x / fs)) + 1) / 2

def add_color(img1, img2):
    added = img1 + img2
    return np.clip(added, 0, 255)

def subtract_color(img1, img2):
    subtracted = img1 - img2
    return np.clip(subtracted, 0, 255)

def product_color(img1, img2):
    producted = img1 * img2
    return np.clip(producted, 0, 255)

def divide_color(img1, img2):
    divided = img1 / img2
    return np.clip(divided, 0, 255)
...
class CascadeDetector:
    ...
    def feature_smoothing(self, face_img, feature_points, mean_range=10):
        face = face_img.copy()
    ...
    def test_mean(self, img):
        ...
        for (x, y, w, h) in faces:
            ...
            noses = self.__get_best_feature_roi(face_gray, nose_cascade, k_mean=3, scale=1.3)
            for (nx, ny, nw, nh) in noses:
                nose = face[ny:ny+nh, nx:nx+nw]
                nose_1 = nose.copy()
                nose_2 = nose.copy()
                mean_range = 5
                # X axis
                prev_x_mean = np.mean(face[ny:ny+nh, nx-mean_range:nx-1], axis=1)
                next_x_mean = np.mean(face[ny:ny+nh, nx+nw+1:nx+nw+mean_range], axis=1)
                prev_x = face[ny:ny+nh, nx-1]
                next_x = face[ny:ny+nh, nx+nw+1]
                diff_x = (next_x_mean - prev_x_mean) / nw
                diff_x_rev = (prev_x_mean - next_x_mean) / nw
                # Y axis
                prev_y_mean = np.mean(face[ny-mean_range:ny-1, nx:nx+nw], axis=0)
                next_y_mean = np.mean(face[ny+nh+1:ny+nh+mean_range, nx:nx+nw], axis=0)
                prev_y = face[ny - 1, nx:nx + nw]
                next_y = face[ny + nh + 1, nx:nx + nw]
                diff_y = (next_y_mean - prev_y_mean) / nh
                diff_y_rev = (prev_y_mean - next_y_mean) /nh

                for i in range(nw):
                    nose_1[:, i] = add_color(prev_x, (diff_x * (i+1)))
                    nose_2[:, (nw-1)-i] = add_color(next_x, (diff_x_rev * (i+1)))
                    nose_x = cv2.addWeighted(nose_1, float(1-(i/nw)), nose_2, float(i/nw), 1)
                face[ny:ny+nh, nx:nx+nw] = nose_x
                face_x = face.copy()
                blur_x = cv2.blur(nose_x, (9, 9))
                for j in range(nh):
                    nose_1[j, :] = add_color(prev_y, (diff_y * (j+1)))
                    nose_2[(nh-1)-j, :] = add_color(next_y, (diff_y_rev * (j+1)))
                    nose_y = cv2.addWeighted(nose_1, float(1-(j/nh)), nose_2, float(j/nh), 1)
                face[ny:ny + nh, nx:nx + nw] = nose_y
                face_y = face.copy()
                blur_y = cv2.blur(nose_y, (9, 9))
                # blur_y = cv2.blur(face_y[ny-5:ny+nh+5, nx-5:nx+nw+5], (9, 9))

                x_size = nw
                y_size = nh
                f = 1
                weights = get_cosine_kernel(x_size, y_size)
                x_weights = np.array([(np.cos(2 * np.pi * f * (i / (x_size-1))) + 1) / 2 for i in np.arange(x_size)])
                #x_weights = np.power(x_weights, 10)
                y_weights = np.array([(np.sin(2 * np.pi * (f/2) * (i / (y_size))) + 0) for i in np.arange(y_size)])
                #y_weights = np.power(y_weights, 10)

                # print(x_fs,x_weight.shape, y_fs,np.reshape(y_weight, (y_fs, 1)).shape)
                #x_y_weight = np.matmul(np.reshape(x_weight, (x_fs, 1)), np.reshape(y_weight, (1, y_fs)))
                # np.savetxt("weights.txt", weights, fmt="%2.4f")
                #
                # print(x_y_weight.shape)
                # print(np.reshape(x_weight, (x_fs, 1)))
                # print(np.reshape(y_weight, (1, y_fs)))

                # for i in range(x_size):
                #     face_1[ny:ny+nh, nx+i] = cv2.addWeighted(face_x[ny:ny+nh, nx+i], float(x_weights[i]),
                #                                                    face_y[ny:ny+nh, nx+i], float(1 - (x_weights[i])), 0)
                # for j in range(y_size):
                #     face_2[ny+j, nx:nx+nw] = cv2.addWeighted(face_y[ny+j, nx:nx+nw], float(y_weights[j]),
                #                                                face_x[ny+j, nx:nx+nw], float(1-(y_weights[j])), 0)

                for i in range(nw):
                    # x = max(i, nw-1-i) - int(nw/2)
                    # pow_val = (x)**2 + 1
                    # print(pow_val, x)
                    pow_val = ((x_weights[i]+1) ** 2)
                    # print(pow_val)
                    nose_1[:, i] = cv2.addWeighted(blur_x[:, i], float(np.power(x_weights[i], pow_val)),
                                                   blur_y[:, i], float(1 - np.power(x_weights[i], pow_val)), 0)
                face[ny:ny + nh, nx:nx + nw] = nose_1
                # blur = cv2.blur(face[ny-mean_range:ny+nh+mean_range, nx-mean_range:nx+nw+mean_range], (math.ceil(nh/10), (math.ceil(nw/10))))
                blur = cv2.medianBlur(face[ny - mean_range:ny + nh + mean_range, nx - mean_range:nx + nw + mean_range],
                                9)
                face[ny - mean_range:ny + nh + mean_range, nx - mean_range:nx + nw + mean_range] = blur
                #     for j in range(nh):
                #         blended = cv2.addWeighted(blur_x[j][i], float(weights[i][j]),
                #                         blur_y[j][i], float(1 - weights[i][j]), 0)
                #         nose_2[j][i] = np.reshape(blended, 3)
                # face[ny:ny+nh, nx:nx+nw] = nose_2.copy()
                for j in range(nh):
                    nose_2[j, :] = cv2.addWeighted(blur_x[j, :], float(1-y_weights[j]),
                                                   blur_y[j, :], float(y_weights[j]), 0)

                for i in range(nw):
                    for j in range(nh):
                        nose_1[j][i] = np.reshape(cv2.addWeighted(blur_x[j][i], (x_weights[i] + y_weights[j])/2,
                                                                  blur_y[j][i], 1- (x_weights[i] + y_weights[j])/2, 0), (3,))

                face[ny:ny + nh, nx:nx + nw] = nose_1
                blur = cv2.blur(face[ny - mean_range:ny + nh + mean_range, nx - mean_range:nx + nw + mean_range],
                                (9, 9))
                face[ny - mean_range:ny + nh + mean_range, nx - mean_range:nx + nw + mean_range] = blur

                cv2.imshow('Nose_X',face_x)
                cv2.imshow('Nose_Y',face_y)
                cv2.imshow('Filled', face)
                cv2.imshow('Cos_x', blur_x)
                cv2.imshow('Cos_y', blur_y)
                # cv2.imshow('Blur', blur)
                #cv2.imshow('Mean_XY', )
                #cv2.imshow('Blended', cv2.addWeighted(face_1, 0.5, face_2, 0.5, 0))
                #cv2.imshow('origin', face)
                cv2.waitKey()
                cv2.destroyAllWindows()
